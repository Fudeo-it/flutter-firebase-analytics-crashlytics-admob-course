...
class SignInBloc extends Bloc<SignInEvent, SignInState> {
    ...
    if (event is PerformSignInEvent || event is PerformSignInWithGoogleEvent) {
      ...
      try {
        if (event is PerformSignInEvent) {
          ...
          if (userCredential.user != null) {
            await _requestPermissionAndUpdateToken(userCredential.user!.uid);
          }
        } else {
      ...
      if (userCredential != null) {
        firebaseAnalytics.logLogin(
            loginMethod:
                event is PerformSignInEvent ? "credentials" : "google");
  ...
  void _updateUserProfile(AuthenticatedState state) async {
    ...
    await _requestPermissionAndUpdateToken(user.uid);
  }

  Future<void> _requestPermissionAndUpdateToken(String uid) async {
    final settings = await firebaseMessaging.requestPermission();
    if (AuthorizationStatus.authorized == settings.authorizationStatus) {
      final token = await firebaseMessaging.getToken();

      if (token != null) {
        await userRepository.update(uid, token: token);
      }
    }
  }
}
